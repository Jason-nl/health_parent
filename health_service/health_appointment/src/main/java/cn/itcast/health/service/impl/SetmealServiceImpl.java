...
@Service
@Transactional
public class SetmealServiceImpl extends ServiceImpl<SetmealMapper, Setmeal> implements cn.itcast.health.service.SetmealService {
    ...
    public SetmealVO findSetMealDetail(int id) {
        // 根据ID查询 setmeal的部分信息
        SetmealVO setmealVO = baseMapper.findSetmealInfoById(id);
        List<CheckGroupVO> checkGroupVOList = checkGroupService.findCheckGroupVoById(id);
        for (CheckGroupVO checkGroupVO : checkGroupVOList) {
           List<CheckItem> checkItemList = checkItemService.findCheckItemListByGid(checkGroupVO.getId());
